    ...
    for(var i = 0; i<=folderData.length-1;i++){
        if(folderData[Number(i)]!=".."){
            if(folderData[Number(i)].includes(".")){
                $('#files').append(`<button style="width:100%;" id="FILE_${folderData[Number(i)]}" onClick='localStorage.setItem("location",folderName+"/"+this.id.replace("_FILE",""));openStickynote(PathToFile(computator,folderName+"/"+button.id.replace("_FILE","")))' class = "FileButton">${folderData[Number(i)]}</button><br>`);
            }else{
